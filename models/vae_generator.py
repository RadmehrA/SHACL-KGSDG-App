...
def factorize_and_initialize_vae(file_path, latent_dim=64):
    ...
    # Build a mapping from local name to full URI for convenience
...
def generate_data_vae_model(model, factorized_data, subject_input, predicate_input, num_samples=1):
    ...
    # Map local name to full URI if needed
    ...
    # Find exact matches in factorized_data
...
class VAE(nn.Module):
    def __init__(self, subject_dim, predicate_dim, object_dim=None, latent_dim=64, numeric=False):
        ...
        # Decoder
        if numeric:
            output_dim = 1  # scalar for numeric
        else:
    ...
    def decode(self, z, x_cond):
        ...
        if self.numeric:
            return self.fc3(h2)  # linear output
        return torch.tanh(self.fc3(h2))  # categorical output
...
# -----------------------------
# Auto-detect categorical/numeric predicates
# -----------------------------
...
# -----------------------------
# Factorize triples and prepare VAE data
# -----------------------------
...
# -----------------------------
# Full pipeline: upload, train, save
# -----------------------------
...
# -----------------------------
# Generate data automatically selecting correct VAE
# -----------------------------
